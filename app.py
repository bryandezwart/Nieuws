...
USER_AGENT = "Mozilla/5.0 (compatible; RadioVoicetrackDashboard/5.0)"
...
def inject_custom_css():
    st.markdown(
        ...
        :root {
            ...
            --rm-bg: #050C16;
            --rm-bg-soft: #091425;
            --rm-panel: rgba(12, 23, 38, 0.92);
            --rm-panel-2: rgba(18, 33, 53, 0.94);
            --rm-border: rgba(255, 185, 92, 0.16);
            --rm-border-strong: rgba(242, 140, 24, 0.32);
            --rm-text: #F7F9FC;
            --rm-muted: #A7B6CB;
            --rm-shadow: 0 20px 40px rgba(0, 0, 0, 0.28);
        }

        .stApp {
            background:
                radial-gradient(circle at top right, rgba(242, 140, 24, 0.10), transparent 24%),
                radial-gradient(circle at top left, rgba(255, 179, 71, 0.06), transparent 18%),
                linear-gradient(180deg, #040A13 0%, #08111F 42%, #0B1527 100%);
            color: var(--rm-text);
        }
        ...
        .rm-hero {
            ...
            background: linear-gradient(145deg, rgba(18, 31, 50, 0.96) 0%, rgba(10, 20, 36, 0.98) 100%);
        ...
        .rm-hero::before {
            ...
            inset: 0;
            background:
                radial-gradient(circle at right top, rgba(242, 140, 24, 0.14), transparent 26%),
                radial-gradient(circle at left bottom, rgba(242, 140, 24, 0.06), transparent 18%);
            pointer-events: none;
        }
        ...
        button[role="tab"][aria-selected="true"] {
            ...
            box-shadow: 0 10px 24px rgba(242, 140, 24, 0.28);
        }

        .stButton > button,
        .stDownloadButton > button {
            background: linear-gradient(180deg, #FFB347 0%, var(--rm-orange) 38%, var(--rm-orange-dark) 100%) !important;
            ...
            box-shadow: inset 0 1px 0 rgba(255,255,255,0.28), 0 10px 24px rgba(242, 140, 24, 0.24) !important;
        }
        ...
        .stDownloadButton > button:hover {
            ...
            box-shadow: inset 0 1px 0 rgba(255,255,255,0.32), 0 14px 30px rgba(242, 140, 24, 0.30) !important;
        }
        ...
        .stSlider [data-baseweb="slider"] div[role="slider"] {
            ...
            box-shadow: 0 0 0 2px rgba(242, 140, 24, 0.18) !important;
        }
        ...
        .stRadio [role="radiogroup"] label[data-baseweb="radio"] div[data-checked="true"],
        ...
        div[data-testid="stExpander"] {
            border: 1px solid var(--rm-border);
            border-radius: 18px;
            background: linear-gradient(180deg, rgba(14, 26, 43, 0.88) 0%, rgba(10, 21, 36, 0.92) 100%);
            overflow: hidden;
        ...
        a { color: var(--rm-orange) !important; font-weight: 600; }
        </style>
        """,
    ...
    st.markdown(
        ...
        <div class="rm-hero">
            ...
            <div class="rm-badge-row">
                ...
                <div class="rm-badge">📰 Opmerkelijk</div>
...
def build_complete_voicetrack(day_obj: date, day_people: List[BirthdayPerson], news_items: List[NewsItem], news_mode: str, script_length: str = "normaal", script_tone: str = "luchtig") -> str:
    ...
    lines = [opener]
    if chosen_people:
        lines.append(f"Op de verjaardagskalender staan vandaag {len(chosen_people)} namen die je mooi kunt meenemen in je break.")
...
def birthdays_tab():
    ...
    for day_iso in sorted(grouped):
        ...
        with st.expander(f"{label} — {len(day_people)} personen • {music_count} muziek", expanded=(day_iso == sorted(grouped)[0])):
            ...
            if st.button("🎙️ Genereer voicetrack", key=f"btn_{day_iso}"):
            ...
            if st.session_state.get(f"show_script_{day_iso}"):
                ...
                st.download_button("⬇️ Exporteer voicetrack", data=script.encode("utf-8"), file_name=f"voicetrack_{day_iso}.txt", mime="text/plain", key=f"dl_{day_iso}")

    csv_df = birthday_dataframe(filtered)
    if not csv_df.empty:
        st.download_button("⬇️ Exporteer verjaardagen", data=csv_df.to_csv(index=False).encode("utf-8-sig"), file_name="jarigvandaag_selectie.csv", mime="text/csv")


def news_tab():
    st.markdown('<div class="rm-subheader">📰 NU.nl Opmerkelijk</div>', unsafe_allow_html=True)
    ...
    st.download_button("⬇️ Exporteer nieuwsitems", data=news_dataframe(filtered).to_csv(index=False).encode("utf-8-sig"), file_name="nu_opmerkelijk_selectie.csv", mime="text/csv")


def app():
    inject_custom_css()
    left, right = st.columns([2.9, 1.7])
    with right:
