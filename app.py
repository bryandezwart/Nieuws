...
def birthday_line_for_script(person: BirthdayPerson, script_tone: str = "luchtig") -> str:
...
def build_complete_voicetrack(
    ...
    if script_tone == "enthousiast":
        ...
        one_birthday_intro = "Er is vandaag een naam op de verjaardagskalender die je er echt even uit kunt pikken."
        multi_birthday_intro = f"Op de verjaardagskalender staan vandaag {len(chosen_people)} namen die je lekker kunt meenemen in je break."
        exact_news_intro = "En dan nog iets opmerkelijks van vandaag, ook zo'n verhaal waarvan je denkt: dat verzin je niet."
        recent_news_intro = "En ook nog een opvallend nieuwtje dat de afgelopen dagen voorbij kwam."
        no_news_short = "Verder kun je hier nog een opvallend nieuwtje aan vastplakken als je wilt."
        no_news_long = "Voor het opmerkelijke nieuws kun je hier later nog een extra opvallend item aan toevoegen."
        outro_short = "En we knallen lekker door met muziek die je pakt."
        outro_normal = "Dat was 'm voor nu, en we gaan natuurlijk snel weer verder met muziek die je pakt."
        outro_long = "Zo heb je meteen een complete, energieke break te pakken. En natuurlijk gaan we verder met muziek die je pakt."
    elif script_tone == "zakelijk":
        opener = f"Het is {weekday} {day_obj.day} {month} en dit is de update van Radio Muziekstad."
        one_birthday_intro = "Er staat vandaag één relevante naam op de verjaardagskalender."
        multi_birthday_intro = f"Op de verjaardagskalender staan vandaag {len(chosen_people)} relevante namen."
        exact_news_intro = "Daarnaast is er ook een opmerkelijk nieuwsitem van vandaag."
        recent_news_intro = "Daarnaast is er ook een opvallend nieuwsitem van de afgelopen dagen."
        no_news_short = "Een aanvullend opmerkelijk nieuwsitem kan hier later nog worden toegevoegd."
        no_news_long = "Voor het opmerkelijke nieuws kan hier later nog een aanvullend item worden toegevoegd."
        outro_short = "We gaan verder met muziek op Radio Muziekstad."
        outro_normal = "Dat was de update voor nu. We gaan verder met muziek op Radio Muziekstad."
        outro_long = "Zo staat er een complete, rustige break klaar. We gaan verder met muziek op Radio Muziekstad."
    else:
        opener = f"Het is {weekday} {day_obj.day} {month} en dit is jouw korte update op Radio Muziekstad."
        one_birthday_intro = "Er staat vandaag één naam op de verjaardagskalender die je eruit kunt lichten."
        multi_birthday_intro = f"Op de verjaardagskalender staan vandaag {len(chosen_people)} namen die leuk zijn om even mee te pakken."
        exact_news_intro = "En ook nog iets opmerkelijks van vandaag."
        recent_news_intro = "En nog iets opvallends dat de afgelopen dagen in het nieuws opdook."
        no_news_short = "Verder kun je hier nog een kort opvallend nieuwtje aan toevoegen."
        no_news_long = "Voor het opmerkelijke nieuws kun je hier later nog een extra luchtig item aan toevoegen."
        outro_short = "En we gaan snel weer verder met muziek die je pakt."
        outro_normal = "Dat was 'm voor nu, en natuurlijk gaan we weer verder met muziek die je pakt."
        outro_long = "Zo heb je in één keer een complete en vloeiende break met zowel verjaardagen als een opvallend nieuwsmoment. Dat was 'm voor nu, en natuurlijk gaan we weer verder met muziek die je pakt."
    ...
    if chosen_people:
        if script_length == "kort":
            if script_tone == "zakelijk":
                lines.append("Kort de verjaardagskalender van vandaag.")
            else:
                lines.append("Even snel naar de verjaardagskalender van vandaag.")
        elif len(chosen_people) == 1:
            lines.append(one_birthday_intro)
        else:
            lines.append(multi_birthday_intro)

        for idx, person in enumerate(chosen_people):
            if script_length == "kort" and idx > 0:
                break
            lines.append(birthday_line_for_script(person, script_tone=script_tone))

    if news_items:
        lines.append(exact_news_intro if news_mode == "exact" else recent_news_intro)
        ...
        lines.append(no_news_short if script_length == "kort" else no_news_long)

    if script_length == "kort":
        lines.append(outro_short)
    elif script_length == "uitgebreid":
        lines.append(outro_long)
    else:
        lines.append(outro_normal)
...
def birthdays_tab():
    ...
    news_items_for_scripts: List[NewsItem] = []
    news_feed_error = None
    try:
        news_items_for_scripts = fetch_nu_rss(NU_RSS_URL)
    except Exception as exc:
        news_feed_error = str(exc)

    if news_feed_error:
        st.caption("Opmerkelijk nieuws kon niet automatisch worden geladen voor de complete dag-voicetrack.")
